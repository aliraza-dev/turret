...
import { incomingCall, privateCall } from '../../redux/actions/dialActions'
            ...
            {             
                Array.from(Array(5).keys()).map(e => (
                    <Row key={e} justify="center">
                        <button key={e * 2 + 1} 
                        className={["speed-dial-button", (callData.call_num_info === (e * 2 + 1) ? "blink" : "", (callData.private_call ? "private-call" : ""))].join(" ")} 
                        onClick={() => writeEvent(DialButtonList[e*2+1][0], e*2+1, DialButtonList[e*2+1][1])}>
                            {DialButtonList[e*2+1][0]}<br />{DialButtonList[e*2+1][1]}
                        </button>
                        <button key={e * 2 + 2} 
                        className={["speed-dial-button", (callData.call_num_info === (e * 2 + 2) ? "blink" : "", (callData.private_call ? "private-call" : ""))].join(" ")} 
                        onClick={() => writeEvent(DialButtonList[e*2+2][0], e*2+2, DialButtonList[e*2+2][1])}>
